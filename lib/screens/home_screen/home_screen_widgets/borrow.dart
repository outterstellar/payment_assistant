...
class _BorrowScreenState extends State<BorrowScreen> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
                              ...
                              onTap: () {
                                Navigator.of(context).push(CupertinoPageRoute(
                                    builder: (context) => InfoScreen(
                                          explanation:
                                              borrows[index].explanation,
                                          name: borrows[index].name,
                                          amount: borrows[index].amount,
                                          value_equals_to_lend: false,
                                          index: index,
                                        )));
                              },
                              leading: CircleAvatar(
                                  child: Text(
                                      borrows[index].name.characters.first)),
                        ...
                        itemBuilder: ((context, index) {
                          return ListTile(
                            onTap: () {
                              Navigator.of(context).push(CupertinoPageRoute(
                                  builder: (context) => InfoScreen(
                                        explanation: borrows[index].explanation,
                                        name: borrows[index].name,
                                        amount: borrows[index].amount,
                                        value_equals_to_lend: false,
                                        index: index,
                            ...
                            leading: CircleAvatar(
                                child:
                                    Text(borrows[index].name.characters.first)),
                            title: Text(borrows[index].name),
                            trailing: Text(borrows[index].amount.toString()),
                            subtitle: Text(returnExplanataion(index)),
                          );
        ...
        future: newborrows == false && datas_first_come2 == false
            ? firestore.collection("users").doc(email).get().then((value) {
                ...
                for (var i = 0; i < ahandayenicikanlendler.length; i++) {
                  borrows.add(Model(
                      name: ahandayenicikanlendler[i]["isim"],
                      amount: double.parse(
                          ahandayenicikanlendler[i]["miktar"].toString()),
                      explanation: ahandayenicikanlendler[i]["aciklama"]));
                }
            ...
            : firestore
                .collection("users")
                .doc(email)
                .update({"borrows": createListFromBorrows()}));
  }
}
...
List createListFromBorrows() {
  ...
  for (var i = 0; i < borrows.length; i++) {
    liste.add({
      "isim": borrows[i].name,
      "miktar": borrows[i].amount,
      "aciklama": borrows[i].explanation
    });
  }
  return liste;
}

String returnExplanataion(index) {
