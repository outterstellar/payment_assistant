...
class _LendScreenState extends State<LendScreen> {
  ...
  Widget build(BuildContext context) {
        ...
        children: [
          Flexible(
            ...
            child: ListView.builder(itemBuilder: ((context, index) {
              return ListTile(
                onTap: (){
                  Navigator.of(context).push(CupertinoPageRoute(builder: (context)=>InfoScreen(explanation: lends[index].explanation, name: lends[index].name, amount:  lends[index].amount,value_equals_to_lend: true,index: index,)));
                },
                leading: CircleAvatar(child: Text(lends[index].name.characters.first)),
    ...
    }),future: newlends==false&& datas_first_come==false?firestore.collection("users").doc(email).get().then((value) {
      List newlendslist=value.data()?["lends"];
      debugPrint("liste"+newlendslist.toString());
      for (var i = 0; i < newlendslist.length; i++) {
        lends.add(Model(name: newlendslist[i]["isim"], amount:double.parse( newlendslist[i]["miktar"].toString()), explanation: newlendslist[i]["aciklama"]));
      }
      debugPrint(lends.toString());
    
      
    }):firestore.collection("users").doc(email).update({"lends":createListFromLends()})
...
List createListFromLends(){
  ...
  for (var i = 0; i < lends.length; i++) {
    list.add({"isim":lends[i].name,"miktar":lends[i].amount,"aciklama":lends[i].explanation});
  }
  return list;
}
